...
class SSPSO(BaseLLH):
    ...
    Theorem 6.3.1: The system converges if c3 < (1 - Ï) / 2
    """
    ...
    def __init__(
        ...
        Args:
            ...
            spatial_weight: Î» weight for spatial regularization term L_spat
            topology_size: k for kÃk communication neighbourhood
    ...
    def _enforce_convergence_bound(self):
        """Enforce Theorem 6.3.1: c3 < (1 - Ï) / 2"""
    ...
    def _compute_spatial_gradient(
        ...
        """
        Compute spatial regularization gradient â_f L_spat.
        
        L_spat = Î£_{i,jâN} exp(-||p_i - p_j||Â²/2ÏÂ²) * I[l_i â  l_j]
    ...
    def _update_topology_best(self, particle_idx: int):
        """Update local best based on kÃk topology (Section 6.3.3)."""
    ...
    def _apply_meta_conditioning(self, meta_features: torch.Tensor):
        ...
        Uses neural network to predict Ï, c1, c2, c3 from meta-features.
        """
...
class SSPSOSpatial(SSPSO):
    """Spatial variant: high spatial regularization (Î» = 2.0)"""
    def __init__(self, **kwargs):
        super().__init__(spatial_weight=2.0, **kwargs)


class SSPSOSpectral(SSPSO):
    """Spectral variant: pure spectral clustering (Î» = 0.1)"""
    def __init__(self, **kwargs):
