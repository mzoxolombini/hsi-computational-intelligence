...
class LipschitzEstimator:
    ...
    Theorem B.4.1: For normalized hyperspectral data p â [0,1]^B,
    the Lipschitz constant satisfies L â¤ 2.
    """
    ...
    def compute(data: torch.Tensor, num_samples: int = 500) -> float:
        ...
        for _ in range(min(num_samples, 10000)):
            ...
            if norm_diff > eps:
                ...
                # L â ||âL_spat(f_i) - âL_spat(f_j)|| / ||f_i - f_j||
                # Theoretical bound: L â¤ 2
...
class ConvergenceValidator:
    ...
    Theorem: System converges if c3 < (1 - Ï) / 2
    ...
    def empirical_bound(self, data: torch.Tensor) -> float:
        ...
        Returns:
            max_c3 = (1 - Ï) / (2 * L) where L is Lipschitz constant
        """
...
class ConvergenceRateAnalyzer:
    ...
    - Convergent region: c3 â [0, 0.25] for Ï=0.5
    - Oscillatory region: c3 â (0.25, 0.35]
...
class GeneralisedConvergenceBound:
    ...
    For data with dynamic range Ï_max, the convergence condition becomes:
    c3 < (1 - Ï) / (2 * Ï_max^2)
    
    Also requires spatial bandwidth scaling: Ï' = Ï * Ï_max
    """
    
    @staticmethod
    def compute_dynamic_range(data: torch.Tensor) -> float:
        """
        Compute Ï_max = max(p) - min(p) for the dataset.
        
        Args:
            data: Hyperspectral data (N, B)
            
        Returns:
            Ï_max: Dynamic range
        """
    ...
    def validate_normalization_requirement(data: torch.Tensor, tolerance: float = 1e-3) -> bool:
        ...
        Returns:
            is_normalized: True if data is within [0,1] Â± tolerance
        """
...
def prove_theorem_6_3_1():
    ...
    Theorem: Given SS-PSO with inertia weight Ï â [0,1] and coefficients c1, c2 > 0,
    the system remains stable and converges to a local optimum if the spatial
    regularization coefficient satisfies: c3 < (1 - Ï) / 2
    ...
    Given SS-PSO with inertia weight Ï â [0,1] and coefficients câ, câ > 0,
    the system remains stable and converges to a local optimum if the spatial
    ...
    recurrence:
        ...
        v_i(t+1) = Ï v_i(t) + u_i(t) + câ â_f L_spat(f_i(t))
    
    where u_i(t) = cârâ(p_i - f_i(t)) + cârâ(g - f_i(t)) is the conventional
    PSO force term.
    ...
    Consider the combined force operator:
        ...
        Î¦(f, v) = Ï v + u(f) + câ â_f L_spat(f)
    
    For SS-PSO to be stable, Î¦ must be a contraction mapping in the Banach
    space (â^{2b}, ||Â·||_â). This requires Lipschitz constant K_Î¦ < 1.
    
    B.4 Lipschitz Constant Derivation
    ---------------------------------
    The Jacobian of Î¦ with respect to state [f, v]^T is:
    
        J_Î¦ = [-(cârâ + cârâ)I_b + câ H_L, Ï I_b]
    
    where H_L is the Hessian of L_spat.
    
    The induced â-norm of J_Î¦ is:
    
        ||J_Î¦||_â = max(||cârâ + cârâ||_â + câ||H_L||_â, Ï)
    
    Given that râ, râ ~ U(0,1) and ||H_L||_â â¤ 2L â¤ 4 (for normalized data):
    
        ||J_Î¦||_â â¤ max(câ + câ + 4câ, Ï)
    ...
    Using the standard PSO parameterization câ = câ = Ï/2 where Ï â 2.05,
    ...
    iteration t satisfies:
        ...
        ||e(t+1)||_â â¤ K_Î¦ ||e(t)||_â
    
    where K_Î¦ = Ï + câ + câ + 2câ < 1 under the conditions of the theorem.
    
    Therefore, the system converges to a local optimum. â
    """
...
def run_empirical_validation(n_runs: int = 10):
    ...
    for c3, info in results.items():
        status = "â STABLE" if info['is_stable'] else "â UNSTABLE"
